...
def build_conv(base_name='i3d', input_type='rgb', kinetics_pretrain=None, mode='global', freeze_affine=True):
...
class ContextNet(nn.Module):
    ...
    def __init__(self, cfg, input_type):
        ...
        self.i3d_conv_context = build_conv(cfg.base_net, self.input_type, cfg.kinetics_pretrain, 'context', self.freeze_affine)
...
class TwoBranchNet(nn.Module):
    ...
    def __init__(self, cfg, cls_only=False, input_type='rgb'):
        ...
        self.i3d_conv = build_conv(cfg.base_net, self.input_type, cfg.kinetics_pretrain, 'global', self.freeze_affine)
    ...
    def forward(self, global_feat, context_feat=None, tubes=None, targets=None):
        ...
        chunks = int(T / self.T)
        chunk_idx = [j*self.T + int(self.T/2) for j in range(chunks)]    # used to index the middle frame of each chunk
        half_T = int(self.T/2)

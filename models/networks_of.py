...
class BaseNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.base_name == "i3d":
            self.base_model = build_base_i3d_of(self.kinetics_pretrain, self.freeze_affine)
        else:
            raise NotImplementedError
...
def build_base_i3d_of(kinetics_pretrain=None, freeze_affine=True):

    print ("Building I3D flow model...")
    i3d = I3D_of(num_classes=400)

    if kinetics_pretrain is not None:
        if os.path.isfile(kinetics_pretrain):
            print ("Loading I3D_of pretrained on Kinetics dataset from {}...".format(kinetics_pretrain))

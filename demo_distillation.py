...
def main():
    ...
    checkpoint_path = '/data/Dan/ava_v2_1/cache/STEP_distillation-max3-i3d-two_branch/checkpoint_best.pth'
    ...
    nets = OrderedDict()
    # backbone network
    args.kinetics_pretrain = args.kinetics_pretrain.split('|')[0]
    ...
    nets['base_net'].load_state_dict(checkpoint['base_net_rgb'])
    if not args.no_context and 'context_net' in checkpoint:
        nets['context_net'].load_state_dict(checkpoint['context_net_rgb'])
    for i in range(args.max_iter):
        pretrained_dict = checkpoint['det_net_rgb%d' % i]

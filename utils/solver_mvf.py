...
def get_params(nets, args):
    ...
    for name in sorted(parameter_dict.keys()):
        ...
        if param.requires_grad:
            # base_model.conv1 or bn1
            if name.find('base_model.0')>-1 or name.find('base_model.1')>-1:
                layer_lr = args.base_lr/8
                if name.find('bias') > -1:

...
from utils.vis_utils import overlay_of
from data.customize_of import CustomizedDataset, detection_collate, WIDTH, HEIGHT
from data.augmentations_of import BaseTransform
...
def main():
    ...
    checkpoint_path = '/data/CLASP-DATA/DEMO-DATA/bestModel/Flows/checkpoint_best_of_v2.pth'
    ...
    args.data_root = 'examples/of_frames' # 'examples' 
    ...
    with torch.no_grad():
        for _, (images, tubes, infos) in enumerate(dataloader):
            ...
            for b in range(len(tubes_nums)):
                ...
                print (info)
                overlay_of(os.path.join(args.data_root, info['video_name'], im_format % info['fid']),
                            os.path.join(args.save_root, info['video_name'], (im_format % info['fid']).replace('.npy','.jpg')),

...
from data.mvf_cls import MVFDataset, detection_collate, WIDTH, HEIGHT, TEM_REDUCE
...
try:
    ...
    args.fp16 = False
    pass
## Img Size of pretrained model and label dictionary
...
def main():
    ...
    train_dataset = MVFDataset(args.data_root, 'train', args.input_type, args.T, args.NUM_CHUNKS[args.max_iter], args.fps, augmentation, stride=1, num_classes=args.num_classes, foreground_only=True)
    val_dataset = MVFDataset(args.data_root, 'val', args.input_type, args.T, args.NUM_CHUNKS[args.max_iter], args.fps, BaseTransform(args.image_size, args.means, args.stds,args.scale_norm), stride=1, num_classes=args.num_classes, foreground_only=True)
    ...
    if args.resume_path is not None:
        # Find the path to the trained model
...
#    writer = SummaryWriter(args.save_root+"summary"+datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S'))
    ...
    while epochs < args.max_epochs:
        for data_id, (images, targets, tubes, infos) in enumerate(train_dataloader):
            ...
            # ROI Pooling
            BT,C,W,H= conv_feat.shape
            conv_feat = conv_feat.view(args.batch_size, -1, C, W, H) # (1,24,1024,14,14)
            ...
            pooled_feat = pooled_feat.view(-1, T_length*TEM_REDUCE, C, W, H) # (4,24,1024,7,7)
            # Two Branch Net
            _,_,_,_, cur_loss_global_cls, _, _ = nets['det_net0'](pooled_feat, tubes=flat_tubes, targets=flat_targets)
...
def validate(args, val_dataloader, nets, iteration=0, iou_thresh=0.5):
    ...
    with torch.no_grad():    # for evaluation
        for num, (images, targets, tubes, infos) in enumerate(val_dataloader):
            ...
            BT,C,W,H= conv_feat.shape
            conv_feat = conv_feat.view(args.batch_size, -1, C, W, H) # (1,24,1024,14,14)
            pooled_feat = nets['roi_net'](conv_feat[:, T_start:T_start+T_length].contiguous(), flat_tubes_all)
            _,C,W,H = pooled_feat.size()
            pooled_feat = pooled_feat.view(-1, T_length*TEM_REDUCE, pooled_feat.size(1), W, H)

            global_prob, _,_,_, _,_,_ = nets['det_net0'](pooled_feat, tubes=None, targets=None)

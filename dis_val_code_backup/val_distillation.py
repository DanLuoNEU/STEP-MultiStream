...
def main():
    ...
    print('Validating STEP on ', train_dataset.name)
    ...
    for i in range(args.max_iter):
        nets_rgb['det_net%d' % i].to('cuda:0')
        nets_rgb['det_net%d' % i].set_device('cuda:0')
        nets_of['det_net%d' % i].to('cuda:0')
        nets_of['det_net%d' % i].set_device('cuda:0')
    ...
    args.resume_path = '/data/Dan/ava_v2_1/cache/STEP_distillation-max3-i3d-two_branch/checkpoint_best.pth'

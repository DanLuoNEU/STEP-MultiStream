...
class ConvertFromInts(object):
    def __init__(self, input_type, scale):
...
class Resize(object):
    ...
    def __call__(self, images, tubes=None, proposals=None):
        ...
        for i in range(images.shape[0]):
            resized_images[i] = cv2.resize(images[i], (self.size[0], self.size[1]))
        # don't need to resize tubes because it's PercentCoords
        return resized_images.astype(np.float32), tubes, proposals
    ...
    def __call__(self, images, tubes=None, proposals=None):
        if random.randint(2):
            ...
            for i in range(boxes.shape[0]):
                ...
                if self.scale == 0:
                    images[:,y1:y2,x1:x2] = np.random.uniform(0,255,(y2-y1,x2-x1,c))
                elif self.scale == 1:
                    images[:,y1:y2,x1:x2] = np.random.uniform(0,1,(y2-y1,x2-x1,c))
                elif self.scale == 2:
                    images[:,y1:y2,x1:x2] = np.random.uniform(-1,1,(y2-y1,x2-x1,c))
    ...
    def __call__(self, images, tubes=None, proposals=None):
        ...
        while True:
            ...
            for _ in range(50):
                ...
                current_tubes[:, :, :2] = np.maximum(current_tubes[:, :, :2],rect[:2])
                # adjust to crop (by substracting crop's left,top)
                current_tubes[:,:, :2] -= rect[:2]

                current_tubes[:,:,2:4] = np.minimum(current_tubes[:,:,2:4],rect[2:])
                # adjust to crop (by substracting crop's left,top)
...
class RandomMirror(object):
    '''None business of input channels, so no need to modify
    '''
    def __init__(self):
...
class PhotometricDistort(object):
    ...
    def __call__(self, images, tubes, proposals=None):
        if self.input_type == 'flow':
            return images, tubes, proposals  # Flow stream cannot do this transformation
        ...
        if self.input_type == 'rgb':
            ...
            ims, tubes, proposals = self.rand_light_noise(ims, tubes, proposals)
            img_ret[:, :, :, 0:3] = ims
            return img_ret, tubes, proposals


class TubeAugmentation(object):
    """TubeAugmentation for input as rgb, flows and stacked (rgb+flows)
    __call__(images,tubes,proposals):
        images: (T,H,W,C), C: 3 for rgb, 2 for flow, 5(rgb(3)+flow(2)) for 2s
    """

    def __init__(self, size=300, scale=1, input_type="rgb",
                 mean=(0, 0, 0, 0, 0), stds=(1, 1, 1, 1, 1),
                 do_flip=False, do_crop=False, do_photometric=False, do_erase=False):
        if input_type == "rgb":
            self.mean = mean[:3]
            self.stds = stds[:3]
        ...
        aug_list += [
            ConvertFromInts(input_type=self.input_type, scale=self.scale),
            ToAbsoluteCoords()
        ]
        ...
        if self.do_erase:
            aug_list += [RandomErase(scale=2)]

        aug_list += [
            ToPercentCoords(),
            Resize(self.size)
            # # Removed because CLASP doesn't have the statistics
...
class BaseTransform(object):
    def __init__(self, size=(400,320), mean=(0, 0, 0, 0, 0), stds=(1, 1, 1, 1, 1), scale=1, input_type='rgb'):
        self.size = size
        if input_type == "rgb":
            self.mean = np.array(mean[:3], dtype=np.float32)
            self.stds = np.array(stds[:3], dtype=np.float32)
        elif input_type == "flow":
            self.mean = np.array(mean[:2], dtype=np.float32)
            self.stds = np.array(stds[:2], dtype=np.float32)
        elif input_type == "2s":
            self.mean = np.array(mean, dtype=np.float32)
            self.stds = np.array(stds, dtype=np.float32)
        ...
        self.augment = Compose([
            ConvertFromInts(input_type=self.input_type, scale=self.scale),
            Resize(self.size)
            # # Removed because CLASP doesn't have those statistics
            # Modify return image.astype(np.float32) in Resize
            # SubtractMeans(self.mean),
            # DivideStds(self.stds)
        ])

...
class ConvertFromInts(object):
    ...
    def __call__(self, images, tubes=None, proposals=None):
        if self.input_type == 'flow':
            ...
            if self.scale == 0:
                images[:,:,:,0:3] = images[:,:,:,0:3].astype(np.float32)
            elif self.scale == 1:
                images[:,:,:,0:3] = images[:,:,:,0:3].astype(np.float32) / 255.   # scale [0,255] to [0,1]
            elif self.scale == 2:
                images[:,:,:,0:3] = np.clip(images[:,:,:,0:3], 0, 255)
                images[:,:,:,0:3] = images[:,:,:,0:3].astype(np.float32)*2/255. - 1.   # scale [0,255] to [-1,1]
            
            if self.input_type == "2s":
                images[:,:,:,3:] = images[:,:,:,3:].astype(np.float32)
...
class TubeAugmentation(object):
    ...
    """
    def __init__(self, size=300, mean=(0,0,0,0,0), stds=(1,1,1,1,1), scale=1, input_type="2s",
        ...
        aug_list += [
            ConvertFromInts(input_type=self.input_type,scale=2),
            ToAbsoluteCoords()
            ]
...
class BaseTransform(object):
    def __init__(self, size=(400,320), mean=(0,0,0,0,0), stds=(1,1,1,1,1), scale=1, input_type='2s'):
        self.size = size
        if input_type == "rgb":
